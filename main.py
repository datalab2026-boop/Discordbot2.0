...
def init_db():
    ...
    try:
        ...
        cur.close()
        conn.close()
        print("✅ [DATABASE] Connection established, table is ready.")
...
async def expadd(interaction: discord.Interaction, amount: int):
    ...
    await interaction.response.send_message(f"✅ Added {amount:,} EXP. New Level: {new_lvl}", ephemeral=True)
...
async def expremove(interaction: discord.Interaction, amount: int):
    ...
    await interaction.response.send_message(f"🔻 Removed {amount:,} EXP. New Level: {new_lvl}", ephemeral=True)
    await refresh_board(interaction.guild)
...
async def expset(interaction: discord.Interaction, amount: int):
    ...
    await interaction.response.send_message(f"⚙️ Experience set to {amount:,}. New Level: {new_lvl}", ephemeral=True)
    await refresh_board(interaction.guild)
